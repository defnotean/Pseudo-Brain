...
1. ~~**B4**~~ — **done 2026-08-17**; pure drop-in, no objective change,
   same discipline as the ensemble control.
...
ensemble control. B1–B3 still wait: reply changes or vetoes any of them
before its implementation commit. Silence is not consent here: unlike
RCQ-v3's delegated window, these were scoped after the delegation was
given, so each remaining proposal waits for an explicit go or for the next
planning pass.
