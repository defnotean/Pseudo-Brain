"""Trusted, once-only Torch runner for RCQ-v3 final qualification.
...
_CLAIM_NAME = "rcq-v3-final.claim.json"
_RECEIPT_NAME = "rcq-v3-final.receipt.json"
_DECISION_LEDGER_NAME = "rcq-v3-final.decisions.jsonl"
_DONOR_LEDGER_NAME = "rcq-v3-final.donors.jsonl"
_RANGE_LEDGER_NAME = "rcq-v3-final.ranges.json"
_REGISTERED_CONFIG_RELATIVE_PATH = Path(
    "brain/configs/training/dgx-rcq-v3-reference.toml"
)
...
def _load_pretraining_pin() -> _PinnedDocument:
    ...
    if (
        ...
        or raw["action"] != "rcq_v3_pin_pretraining_v1"
        or raw["qualification_id"] != "rcq_v3_reference_v1"
...
def _load_final_authorization() -> _PinnedDocument:
    ...
    if (
        type(raw["schema_version"]) is not int
        or raw["schema_version"] != 1
        or raw["action"] != "rcq_v3_authorize_final_v1"
        or raw["qualification_id"] != "rcq_v3_reference_v1"
        or raw["range_claim_id"] != _final_range_claim_id()
    ):
    ...
    if (
        ...
        != "final-claims/preclaim-readiness/rcq-v3-reference-v1.json"
    ):
...
def _resolve_release() -> tuple[Path, Path, Path, Path]:
    ...
    expected_registration = _plain_resolve(
        release / _REGISTRATION_RELATIVE_PATH,
        name="fixed RCQ-v3 registration",
        kind="file",
    )
    if config != expected_config:
        raise RCQInputError("config_path is not the fixed RCQ-v3 reference config")
    if registration != expected_registration:
        raise RCQInputError("registration_path is not the fixed RCQ-v3 registration")
...
def _require_registered_config(
    ...
    if len(config.stages) != 2:
        raise RCQInputError("RCQ-v3 requires exactly two optimizer stages")
...
def _readiness_path(context: _PreclaimContext) -> Path:
    ...
    if type(relative) is not str or relative != (
        "preclaim-readiness/rcq-v3-reference-v1.json"
    ):
...
def _load_readiness_receipt_for_verification(
    ...
    if (
        ...
        or readiness["evaluator_id"] != "rcq_v3_final_v1"
...
def _prepare_receipt_verification_bindings() -> Mapping[str, object]:
    ...
    readiness_path = _plain_resolve(
        claim_registry / "preclaim-readiness" / "rcq-v3-reference-v1.json",
        name="preclaim readiness receipt",
        kind="file",
    )
...
def _verify_rcq_v2_final_receipt() -> Mapping[str, object]:
    ...
    try:
        ...
        if not entries:
            return _deep_freeze(
                {
                    "action": "rcq_v3_verify_receipt_v1",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run trusted RCQ-v3 preclaim, once-only final evaluation, or "
