...
def train_and_eval_variant(
    ...
    *,
    ...
    # Pre-seed buffer with curriculum dataset
        ...
        if optimizer is not None and len(distiller.buffer) > 0:
            ...
            for _ in range(updates_per_iter):
                ...
                for seq in batch.sequences:
                    ...
                    # Directional goal vector targets based on action movement
    ...
    return {
        ...
        "diagnostic_3seed": {
            ...
            "wall_recovery_latency": round(wall_recovery_3, 2),
        },
    }
...
def main() -> None:
    ...
    # Variant F: Counterfactual Foresight
    res_f = train_and_eval_variant(
        "F_counterfactual_foresight",
        use_counterfactual=True,
        use_topological_goal=False,
        dagger_iters=args.dagger_iters,
        updates_per_iter=args.updates_per_iter,
        eval_ticks=args.eval_ticks,
    )
    results["F_counterfactual_foresight"] = res_f

    # Variant G: Counterfactual Foresight + Topological Goal Routing
    res_g = train_and_eval_variant(
        "G_counterfactual_topological",
        use_counterfactual=True,
        use_topological_goal=True,
        dagger_iters=args.dagger_iters,
        updates_per_iter=args.updates_per_iter,
        eval_ticks=args.eval_ticks,
    )
    results["G_counterfactual_topological"] = res_g

    # Print Summary Table
    print("\n" + "=" * 90)
    print("      TOPOLOGICAL GOAL & COUNTERFACTUAL FORESIGHT COMPARISON TABLE")
    print("=" * 90)
    print(f"{'Variant':<35} | {'20-Seed Pellets':<18} | {'20-Seed Total Catches':<22}")
    print("-" * 90)
    for k, v in results.items():
        print(f"{k:<35} | {v['held_out_20seed']['mean_pellets']:<18.2f} | {v['held_out_20seed']['total_catches']:<22}")
    print("=" * 90)

    out_path = pathlib.Path("artifacts/topological_goal_experiment_results.json")
