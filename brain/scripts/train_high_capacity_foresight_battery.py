"""High-Capacity Balanced Foresight Training & Multi-Seed Verification Battery.

Objectives:
1. Drive Hazard Choice-Point AUROC across ALL 5 seeds (42-46) to >= 85-90%+
2. Verify uniform closed-loop catch reduction across all 5 seeds under the Grounded Lookahead Planner.
"""
...
def train_high_capacity_seed(train_seed: int, num_dagger_iters: int = 4, updates_per_iter: int = 24) -> IreneBrainModel:
    """Train IreneBrainModel with high-capacity 5-branch geometric hazard supervision."""
    ...
    training_config = TrainingConfig(
        schema_version=1,
        run=RunConfig(
            name=f"high-cap-seed{train_seed}",
            seed=train_seed,
    ...
    # Auxiliary loss with positive hazard reweighting
        ...
        if optimizer is not None and len(distiller.buffer) > 0:
            ...
            for _ in range(updates_per_iter):
                ...
                for seq in batch.sequences:
                    ...
                    for a_i in range(5):
                        ...
                        if player is not None and ghosts:
                            ...
                            if min_g_dist <= 1:
                                ...
                                haz_val = 0.0
                            esc_val = float(min_g_dist)
                        else:
                            haz_val = 0.0
                            esc_val = 5.0

                        haz_tgt = torch.tensor([[[haz_val], [haz_val * 0.7], [haz_val * 0.5]]], dtype=torch.float32, device=device)
                    ...
                    loss = cog_out.total_loss + 3.0 * total_cf_loss
...
def main() -> None:
    print("=" * 80)
    print("PSEUDO-BRAIN: HIGH-CAPACITY BALANCED FORESIGHT BATTERY (SEEDS 42-46)")
    print("Standard: True Choice-Point Hazard Separation & Multi-Seed Replication")
    print("=" * 80)

    results = {}

    for seed in TRAIN_SEEDS:
        print(f"\n--- Training High-Capacity Seed {seed} ---")
        t0 = time.perf_counter()
        model = train_high_capacity_seed(seed, num_dagger_iters=4, updates_per_iter=24)
        ...
        print(f"Evaluating Grounded Planner for Seed {seed}...")
    ...
    out_file = pathlib.Path("docs/runs/2026-08-19-high-capacity-foresight-battery.json")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 80)
    print("SUMMARY OF 5-SEED HIGH-CAPACITY FORESIGHT BATTERY")
    print("=" * 80)
    for seed, r in results.items():
        ...
        print(f"Seed {seed}: Direct Catches = {d_cat:3d} -> Planner Catches = {p_cat:3d} ({red_pct:+.1f}% reduction) | AUROC = {r['diagnostics']['pairwise_ranking_auroc']:.1f}% | Margin = {r['diagnostics']['choice_point_discrimination_margin']:+.4f}")
