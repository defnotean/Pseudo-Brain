"""Bounded Phase 2.5 Gate 6 probe: thought-mediated K=32 vs matched Proposal-GRU.

Uses the committed HEAD train/eval recipe (300 steps, 5 families, 5 seeds).
Does not run the full thought-mediated campaign. Does not touch RCQ.
"""
...
# python3 -I ignores PYTHONPATH; pin the snapshot src explicitly.
for _candidate in (
...
def find_param_matched_k32_width(target_params: int) -> int:
    """Search K=32 core_width so parameter count matches the GRU envelope.

    The committed HEAD width map (K=32 W=32) is not parameter-matched to
    ProposalGRUBaseline(hidden_dim=180) after the consequence actuator landed.
    Gate 6 requires equal parameters; this search stays on HEAD constructors.
    """
    routed_neighbors = 2
    best_w = 32
    best_diff = float("inf")
    ...
    if rel > PARAM_MATCH_TOLERANCE:
        raise RuntimeError(
            f"K=32 width search could not match GRU envelope {target_params}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 2.5 Gate 6 matched Proposal-GRU probe")
    ...
    parser.add_argument("--output-json", type=str, default="gate6_results.json")
    args = parser.parse_args()

    device = torch.device(args.device)
    print("=== Phase 2.5 Gate 6: thought-mediated K=32 vs matched Proposal-GRU ===")
    print(f"device={device} torch={torch.__version__} cuda={torch.cuda.is_available()}")
    print(f"train_steps={args.train_steps} seeds={SEEDS} episodes_per_world={args.episodes_per_world}")

    gru_probe = ProposalGRUBaseline(hidden_dim=GRU_HIDDEN)
    gru_params = count_params(gru_probe)
    matched_width = find_param_matched_k32_width(gru_params)
    ...
    for seed in SEEDS:
        ...
        train_thought_mediated_model(pb, device=device, training_steps=args.train_steps)
        pb_eval = evaluate_all_families(pb, seed=seed, episodes=args.episodes_per_world, device=device)
        ...
        normal_b = evaluate_closed_loop_model(pb, family_b, seed=seed, episodes=args.episodes_per_world, device=device)
        knockout_b = evaluate_closed_loop_model(
            pb, family_b, seed=seed, episodes=args.episodes_per_world, device=device, intervention_mode="zero_knockout"
        )
        ...
        train_proposal_gru_baseline(gru, device=device, training_steps=args.train_steps)
        gru_eval = evaluate_all_families(gru, seed=seed, episodes=args.episodes_per_world, device=device)
    ...
    gate_pass = bool(adv_iqm >= GATE6_MARGIN and knockout_ok and not gru_wins_or_ties)
    if gru_wins_or_ties:
        verdict = "FAIL"
        claim = False
    ...
    results = {
        "probe_id": "dgx-gate6-matched-gru-v1",
