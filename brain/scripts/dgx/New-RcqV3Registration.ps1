...
$config = Join-Path $repoRoot 'brain\configs\training\dgx-rcq-v3-reference.toml'
$registration = Join-Path $repoRoot 'registrations\rcq-v3-reference-v1.json'
if (-not (Test-Path -LiteralPath $config -PathType Leaf)) {
    throw 'The fixed RCQ-v3 reference configuration is missing.'
}
...
$helper = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..\run_rcq_v3_registration.py')).Path
...
if ($PlanOnly) {
    Write-Host 'PLAN action=create-target-blind-rcq-v3-registration output=registrations/rcq-v3-reference-v1.json isolated_python=-I-B helper=brain/scripts/run_rcq_v3_registration.py'
    return
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Target-blind RCQ-v3 registration failed with exit code $LASTEXITCODE."
}
