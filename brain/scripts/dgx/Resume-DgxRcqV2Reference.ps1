...
if (-not $AcknowledgeDetached) {
    throw 'Reference resume is fixed to Tmux and requires -AcknowledgeDetached. No persistent job is started implicitly.'
}
$target = Resolve-DgxSshTarget -SshTarget $SshTarget
Invoke-DgxRemoteScript `
    -TargetInfo $target `
    -ScriptPath (Get-DgxRemoteHelperPath) `
    -RemoteArguments @('rcq_v2_reference_resume_v1') `
    -PlanOnly:$PlanOnly
