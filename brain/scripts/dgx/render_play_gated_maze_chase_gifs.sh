#!/usr/bin/env bash
# CPU-only maze-chase neural GIF render. Do not pass --gpus. Do not chmod.
set -euo pipefail

workspace="${1:-$HOME/projects/pseudo-brain}"
release_id="${2:-r20260818t195814z-872818a4fa68}"
image="${3:-177a406d7cb2}"
name='play-gated-cpu-render-turn-weighted-v1'
...
docker run --rm --pull never \
  ...
  'python3 -I /workspace/farm/render_maze_chase_neural_replay.py --config /workspace/repo/brain/configs/training/dgx-play-maze-chase-distill-turn-weighted.toml --checkpoint /workspace/ckpt/checkpoints/step-00000032.pt --out-dir /workspace/run --seeds 5,9 --stem turn-weighted-v1' \
  2>&1 | tee "$log_path"
