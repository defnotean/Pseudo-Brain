...
from resolve_broad_policy_v2 import resolve_completed_checkpoint
...
def main():
    ...
    manifest=args.root/'evaluation-source-hashes.json'
    ...
    report={'status':'running','models':{},'source_manifest_sha256':digest(manifest),
            ...
            'provenance':provenance(train_seed=1987,eval_seed=198,bank_digest=BANK['requests.jsonl'],deterministic=False),
    ...
    for name in ('recurrent','transformer'):
        ...
        command=[sys.executable,str(args.root/'evaluate_broad_tool_policy_v2.py'),'--model',name,
