"""Prepare the registered v5 bank remotely; this script never trains a model."""
import argparse
...
from irene_brain.data.signature_conditioned_code import signature_conditioned_exchange
...
V4_DEV_HASH = 'f95f6a2f491db9df37f8682283a49605af68da62e5d0274e5d33a7e7815388b5'
...
def main():
    ...
    parser.add_argument('--prior-foundation-development',type=Path,required=True)
    args = parser.parse_args()
    torch.set_num_threads(1)
    args.output.mkdir(parents=True,exist_ok=False)
    report = {'status':'running','sources':SOURCES,'datasets_version':datasets.__version__,
              'seed':198,'candidate_rows_per_hf_source':131072,'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
              'tokenizer_sha256':TOKENIZER_HASH,'gsm_train_sha256':GSM_HASH,
              'requests_sha256':REQUEST_HASH,'prior_development_gzip_sha256':OLD_DEV_HASH,'v4_development_gzip_sha256':V4_DEV_HASH,
              'partitions':{},'limits':'Exact normalized isolation only; not semantic paraphrase certification or frontier qualification.'}
    report['runtime_provenance'] = provenance(model=None,train_seed=198,eval_seed=None,bank_digest=None,deterministic=False)
    report['code_format'] = 'signature_conditioned_v1'
    ...
    try:
        ...
        v4_dev = load_jsonl_bytes(gzip.decompress(verified_bytes(args.prior_foundation_development,V4_DEV_HASH)))
        assert len(old_dev)==192 and len(v4_dev)==384
        old_dev = old_dev+v4_dev
        ...
        with (args.output/'exclusions.jsonl').open('x') as exclusion_log:
            def consider(raw,task,metadata):
                record = None
                try:
                    record = signature_conditioned_exchange(raw) if task=='code' else complete_exchange(raw,task)
            ...
            for name,config,split,revision,task in SOURCES:
                ...
                observed = 0
                for index,raw in enumerate(stream.shuffle(seed=198,buffer_size=4096).take(131072)):
                ...
                assert observed == 131072, 'Source exhausted before registered candidate count'
        ...
        for partition in ('train','development'):
            ...
            for task in ('language','code','reasoning'):
                quota = (4096 if task=='reasoning' else 32768) if partition=='train' else 512
