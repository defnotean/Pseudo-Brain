"""Bucketed v2 comparison; launch only after the separately registered GPU gate."""
import argparse
import gc
import gzip
import hashlib
...
def encode_rows(rows, tokenizer):
    ...
    for row in rows:
        ...
        assert prompt_end > 0
        padded_ids, padded_labels = bucket_training_example(block.input_ids[None], block.labels[None])
        result.append((row, padded_ids, padded_labels, block.input_ids[None, :prompt_end]))
    return result
...
def main():
    ...
    try:
        ...
        provenance['registration_hashes'] = {name: hashlib.sha256((source_root/'experiments'/name).read_bytes()).hexdigest()
            for name in ('broad_pilot_v1_registration.md', 'broad_pilot_v1_precision_addendum.md', 'broad_bucket_v2_registration.md', 'bucket_gpu_gate_registration.md')}
