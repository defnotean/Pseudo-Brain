"""Recurrent-only length buckets; transformer inputs remain the frozen v1 path."""
import argparse
import gc
import gzip
import hashlib
...
def pilot_loss(model, ids, labels, prompt):
    if model.architecture == ParallelDepthModel.architecture:
        ids, labels = bucket_training_example(ids, labels)
    elif model.architecture != ComparisonTransformer.architecture:
        raise ValueError('Unknown comparison architecture')
    return model.language_loss(ids, labels, prompt_tokens=prompt)
...
def evaluate(model, examples, status, name, stage):
    ...
    with torch.no_grad():
        for index, (row, ids, labels, prompt) in enumerate(examples):
            loss = pilot_loss(model, ids.cuda(), labels.cuda(), prompt.cuda()).item()
    ...
    try:
        with (destination/'training.jsonl').open('x') as log:
            for index, (row, ids, labels, prompt) in enumerate(train):
                ...
                loss = pilot_loss(model, ids.cuda(), labels.cuda(), prompt.cuda())
...
def main():
    ...
    try:
        ...
        provenance['registration_hashes'] = {name: hashlib.sha256((source_root/'experiments'/name).read_bytes()).hexdigest()
            for name in ('broad_pilot_v1_registration.md', 'broad_pilot_v1_precision_addendum.md', 'recurrent_bucket_v3_registration.md')}
