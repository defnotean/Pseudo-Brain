"""V2 native gate: chunked recurrent and full transformer reference readout."""
import argparse
import gc
import gzip
import hashlib
            ...
            for source,example in zip(rows,examples):
                ...
                for key,param in model.named_parameters():
                    ...
                    if param.grad is not None:
                        ...
                        try:
                            torch.testing.assert_close(param.grad,gradients[key],atol=atol,rtol=rtol)
                        except AssertionError as exc:
                            raise AssertionError(key+': '+str(exc)) from exc
                ...
                case={'record_sha256':source['record_sha256'],'input_tokens':encoded['input_ids'].numel(),
                      'supervised_tokens':int(active.sum()),'reference_loss':reference_value,'actual_loss':actual.item(),
                      'max_gradient_absolute_error':maximum,'parameter_gradients_checked':participating,
                      'peak_cuda_allocated_bytes':torch.cuda.max_memory_allocated(),'status':'passed'}
